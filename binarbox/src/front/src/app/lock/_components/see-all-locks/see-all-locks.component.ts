...
@Component({
    selector: 'see-all-locks',
    templateUrl: './see-all-locks.component.html',
    styleUrls: ['./see-all-locks.component.scss']
})

export class SeeAllLocksComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.smallwidth = this.imageWidth * 0.0625/2.5;
        ...
        this.panelSections = [
            ...this.generateSectionsRange(firstRowOptions, [1, 40]),
            ...this.generateSectionsRange(secondRowOptions, [41, 80]),
        ];

        for (var i = 1; i <= this.panelSections.length; i +=4) {
            let coords = this.panelSections[i - 1].coords;
            let order = Math.ceil(Math.random() * 100 % 16);
            order.toString();
            let url = "url(../../../../assets/images/locks/lock";
            url = url + order + ".png)";
            this.panelSections1.push({key: i, coords: coords, url: url});
        }
        ...
        for (var i = 2; i <= this.panelSections.length; i +=4) {
            ...
            let url = "url(../../../../assets/images/locks/lock";
            url = url + order + ".png)";
            this.panelSections2.push({key: i, coords: coords, url: url});
        }
        ...
        for (var i = 3; i <= this.panelSections.length; i +=4) {
            ...
            let url = "url(../../../../assets/images/locks/lock";
            url = url + order + ".png)";
            this.panelSections3.push({key: i, coords: coords, url: url});
        }
        ...
        for (var i = 4; i <= this.panelSections.length; i +=4) {
            ...
            let url = "url(../../../../assets/images/locks/lock";
            url = url + order + ".png)";
            this.panelSections4.push({key: i, coords: coords, url: url});
        }
